...
uint3 dispatchThreadID : SV_DispatchThreadID)
{
    ...
    if (MousePosX % MAX_THREAD == groupThreadID.x && MousePosY % MAX_THREAD == groupThreadID.y)
    {
        ...
        float _nowX = MousePosX / ClientWidth;
        float _nowY = MousePosY / ClientHeight;
        
        float3 _viewRay = 0.0; 
        _viewRay.x = lerp(-halfWidth, halfWidth, _nowX);
        _viewRay.y = lerp(-halfHeight, halfHeight, _nowY);
        _viewRay.z = farZ;
        
        // 노말의 음수값을 양수로 사상했으므로 다시 원래 값으로 돌려주는 작업
        float _opaqueLinearDepth = gInputTexture2[_currPixel].w / farZ;
        float _transparentLinearDepth = gTransparentTexture2[_currPixel].w / farZ;
        
        // near에 가까울수록 값이 낮다
        if (_opaqueLinearDepth < _transparentLinearDepth || _transparentLinearDepth == 0.0)
        {
            float3 _opaqueViewSpacePos = _opaqueLinearDepth * _viewRay;
            float4 _opaquePosition = mul(float4(_opaqueViewSpacePos, 1.0), viewInv);
        ...
        {
            float3 _transparentViewSpacePos = _transparentLinearDepth * _viewRay;
            float4 _transparentPosition = mul(float4(_transparentViewSpacePos, 1.0), viewInv);
