...
public:
	...
	virtual bool IsAttackPartner();								// 조수 공격상태 판단
	virtual bool IsAttackPlayer();								// 플레이어 공격상태 판단
	...
	virtual bool AwakenSight();									// 시야 발견 상태
	virtual bool PostAwakenSight();								// 시야 발견 후 처리
	virtual bool MoveToTarget();								// 타겟 추적

	virtual bool AttackToPlayer() abstract;
	virtual bool DamageToPlayer() abstract;

	virtual bool AttackToPartner();
...
public:
	...
	virtual void FinshDie();									// Die 애니메이션 재생후 Dead로 상태전환 하는 함수
	...
	virtual void PostAssassinated();							// 피암살(애니메이션 종료)후 처리
