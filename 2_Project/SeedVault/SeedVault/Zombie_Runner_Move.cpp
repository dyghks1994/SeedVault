...
bool Zombie_Runner_Move::FindPlayer()
{
	...
	if (UtilityFunction::IsTargetInDetectionRange(*m_pMyObject, *m_Player, m_DetectionRange))
	{
		...
		{
			/// 시야각 안에 있으면 중간에 가로막는게 없는지 검사
			SimpleMath::Vector3 _forward = m_Transform->m_RotationTM.Forward();
				
			SimpleMath::Vector3 _ori = m_Transform->m_WorldPosition + _forward * (m_PhysicsActor->GetBoundingOffset().Forward + 0.1f);
			SimpleMath::Vector3 _dir = m_Player->m_Transform->m_WorldPosition - _ori;
			_dir.Normalize();

			float _dist = m_DetectionRange;

			RaycastHit _hit = DLLEngine::CheckRaycastHit(_ori, _dir, _dist);
			if(_hit.actor != nullptr && _hit.actor->GetMyObject() == m_Player)
			{
				/// 타겟을 찾음!
				// 처음 발견했을 때 
...
bool Zombie_Runner_Move::FindPartner()
{
	...
	if (UtilityFunction::IsTargetInDetectionRange(*m_pMyObject, *m_Partner, m_DetectionRange))
	{
		...
		if (IsTargetInViewSight(*m_Partner) == true)
		{
			...
			SimpleMath::Vector3 _ori = m_Transform->m_WorldPosition + _forward * (m_PhysicsActor->GetBoundingOffset().Forward + 0.1f);
			SimpleMath::Vector3 _dir = m_Partner->m_Transform->m_WorldPosition - _ori;
			_dir.Normalize();
			
			float _dist = m_DetectionRange;
			
			RaycastHit _hit = DLLEngine::CheckRaycastHit(_ori, _dir, _dist);
			if (_hit.actor != nullptr && _hit.actor->GetMyObject() == m_Partner)
			{
				/// 타겟을 찾음!
				// 처음 발견했을 때 
