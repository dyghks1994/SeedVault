...
bool Enemy_Move::IsAttackPartner()
{
	return false;
}

bool Enemy_Move::IsAttackPlayer()
{
...
bool Enemy_Move::AwakenSight()
{
	return false;
}

bool Enemy_Move::PostAwakenSight()
{
	return false;
}

bool Enemy_Move::MoveToTarget()
{
	return false;
}

bool Enemy_Move::AttackToPartner()
{
	return false;
}
...
bool Enemy_Move::HearSound(GameObject* obj, SoundEvent* sound)
{
		...
		&& (m_State & State::eHunt) == false )
	{
		/// 들리는가 판별
		// 사운드 거리안에 있는가? 
		if (UtilityFunction::IsTargetInDetectionRange(m_Transform->m_Position, _playPosition, sound->GetEventDistance()))
		{
			float _dist = sound->GetEventDistance() + 0.1f;
...
bool Enemy_Move::Explore()
{
	if (m_State & State::eExplore)
	{
		...
		// 도착 판정
		float _dist = SimpleMath::Vector3::Distance(m_Transform->m_Position, m_HearSoundPosition);
		if (_dist < 0.25f)
		{
			// 리턴상태로 변경
			m_State & ~State::eExplore;
...
/// 피암살 후 처리
/// </summary>
void Enemy_Move::PostAssassinated()
{
	// 피암살 상태 해제
