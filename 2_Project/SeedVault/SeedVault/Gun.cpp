	...
	: Gun(type, inventory)
{
	...
	m_Damage = 2.0f;
	m_ShootRange = 100.0f;
	m_ShootCoolTime = 2.2f;
	m_ReloadSpeed = 2.5f;
	m_MaxAmmoCount = 4;
	m_CurrentAmmoCount = 4;
}
...
bool LongGun::Shoot()
{
	if (Gun::Shoot() == true)
	{
		...
		{
			_MuzzleFlash->SetMuzzleFlashType(MuzzleFlash::Type::eRifle);
			_MuzzleFlash->Fire();
		}

		// 격발 사운드 이벤트
		auto* _soundEvent = m_pAudio->PlayEvent("event:/Player/Player_Rifle"); 
		...
		// 플레이어 애니메이션 변경
		PlayerController* _playerController = DLLEngine::FindGameObjectByName("Player")->GetComponent<PlayerController>();
		_playerController->m_bShoot = true;

		Animator* _playerMeshAnimator = DLLEngine::FindGameObjectByName("Player_Mesh")->GetComponent<Animator>();
		_playerMeshAnimator->SetOverrideAnimLayer(CL::ResourcePath::ANIM_LAYER_PLAYER_RIFLE_SHOOT, true);
...
bool HandGun::Shoot()
{
	if (Gun::Shoot() == true)
	{
		...
		if (_MuzzleFlash != nullptr)
		{
			_MuzzleFlash->SetMuzzleFlashType(MuzzleFlash::Type::eHandgun);
			_MuzzleFlash->Fire();
		}

		// 격발 사운드 이벤트
		auto* _soundEvent = m_pAudio->PlayEvent("event:/Player/Player_Pistol");
		...
		// 카메라 총기 반동
		GameObject* _CameraParent = DLLEngine::FindGameObjectByName("CameraParent");;
		OrbitCamera* _OrbitCamera = _CameraParent->GetComponent<OrbitCamera>();
		_OrbitCamera->SetRecoil();
