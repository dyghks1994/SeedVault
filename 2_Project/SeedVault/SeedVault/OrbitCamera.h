...
private:
	...
	// 총기반동	중인가?
	bool m_bShootRecoil;
	SimpleMath::Vector2 m_ShootReciolOffset;	// 총기반동 시킬 오프셋 값

	// 
	float m_TotalTime;		// 반동 + 회복 시간
	float m_RecoilTime;		// 총을 쏘고 반동으로 위로 올라가는 시간
	float m_RecoverTime;	// 위에서 다시 아래로 내려오는(반동 잡는) 시간
	float m_RecoverRatio;
