...
ID3D11BlendState* RasterizerState::m_alphaEnableBlendingStateUICombine = nullptr;
...
void RasterizerState::CreateRasterizerState(ID3D11Device* pDevice)
{
	...
	//blendStateDescription2.AlphaToCoverageEnable = FALSE; // -> 안티 앨리어싱 하는놈 
	...
	//blendStateDescription2.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_SRC_ALPHA;
	...
	//blendStateDescription3.AlphaToCoverageEnable = FALSE; // -> 안티 앨리어싱 하는놈 
	blendStateDescription3.RenderTarget[0].BlendEnable = TRUE;
	//blendStateDescription3.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;
	...
	//blendStateDescription3.RenderTarget[0].BlendOp = D3D11_BLEND_OP_MAX;

	//blendStateDescription3.RenderTarget[0].SrcBlendAlpha = D3D11_BLEND_SRC_ALPHA;
	...
	blendStateDescription3.RenderTarget[0].DestBlendAlpha = D3D11_BLEND_ZERO;
	blendStateDescription3.RenderTarget[0].BlendOpAlpha = D3D11_BLEND_OP_ADD;

	blendStateDescription3.RenderTarget[0].RenderTargetWriteMask = D3D11_COLOR_WRITE_ENABLE_ALL;

	// 설명을 사용하여 혼합 상태를 만듭니다.
	pDevice->CreateBlendState(&blendStateDescription3, &m_alphaEnableBlendingStateUICombine);
...
void RasterizerState::DestroyRasterizerState()
{
	...
	ReleaseCOM(m_alphaEnableBlendingStateUICombine);
