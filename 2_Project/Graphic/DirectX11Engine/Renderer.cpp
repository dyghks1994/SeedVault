...
bool Renderer::Initialize(int hwnd, int width, int height)
{
	...
	m_SSScreen = std::make_unique<class SSScreen>();
	m_SSScreen->Initialize(m_pDevice);

	m_SSDManager = std::make_unique<class SSDManager>();
	m_SSDManager->Initialize(m_SSScreen);
...
void Renderer::LoadResource()
{
	...
	_storeShader(m_ShaderIndex_V, ENUM(eSHADER::VS_SSD), "vs_SSD.cso");
	_storeShader(m_ShaderIndex_V, ENUM(eSHADER::PS_SSD), "ps_SSD.cso");
	...
	_storeShader(m_ShaderIndex_V, ENUM(eSHADER::PS_2D_NoGammaCor), "ps_2dNoGammaCor.cso");
	...
	const DirectX::XMMATRIX& projTM, float fovy, float farZ)
{
	...
	ConstantBufferManager::GetPSCamera()->data.CameraPos.w = farZ;
	...
	if (m_CameraFovY != fovy)
	{
		...
		DirectX::XMFLOAT4* _tempFloat4 = m_SSScreen->m_FrustumFarCorner;

		_tempFloat4[0].x = -halfWidth; _tempFloat4[0].y = -halfWidth; _tempFloat4[0].z = farZ; _tempFloat4[0].w = 0.0f;
		_tempFloat4[1].x = -halfWidth; _tempFloat4[1].y = +halfWidth; _tempFloat4[1].z = farZ; _tempFloat4[1].w = 0.0f;
		_tempFloat4[2].x = +halfWidth; _tempFloat4[2].y = +halfWidth; _tempFloat4[2].z = farZ; _tempFloat4[2].w = 0.0f;
		_tempFloat4[3].x = +halfWidth; _tempFloat4[3].y = -halfWidth; _tempFloat4[3].z = farZ; _tempFloat4[3].w = 0.0f;

		for (unsigned int i = 0; i < 4; i++)
		{
			ConstantBufferManager::GetPSAmbientBuffer()->data.frustumCorner[i] = _tempFloat4[i];
		}
	...
	const DirectX::SimpleMath::Matrix& worldTM)
{
	...
	default:
	{
		...
		if (_nowModel->m_pSkeleton != nullptr)
		{
			...
			m_LightManager->BuildShadowMap(m_pDeviceContext, _nowLight,	_nowModel, worldTM, true);
		}
		else
		{
			...
			for (auto& _nowNode : *m_pResourceManager->GetModelMesh(modelIndex)->m_pNodeData_V)
			{
				...
				if (m_FrustumVolume->GetFrustum().Intersects(_nowNode->m_BoundingVolume->GetBoundingSphere()) == true)
				{
					...
					m_LightManager->BuildShadowMap(m_pDeviceContext, 
						_nowLight,	_nowModel, worldTM, false);
				}
			}
		}
...
void Renderer::RenderQueueProcess()
{
	...
	{// 데칼 패스
		m_pDeviceContext->OMSetRenderTargets(1,
			m_Deferred->GetDrawLayers()[ALBEDO_MAP]->GetRenderTargetViewAddressOf(), nullptr);
		m_SSDManager->DrawSSD(
			m_pDeviceContext,
			m_Deferred->GetDrawLayers()[NORMALDEPTH_MAP]->GetShaderResourceView(),
			VS_SHADER(eSHADER::VS_SSD), PS_SHADER(eSHADER::PS_SSD),
			m_ViewTM, m_ProjectionTM);
	}

	{
		m_SSAOManager->ComputeSsao(m_pDeviceContext,
			m_ProjectionTM, m_Deferred->GetDrawLayers()[NORMALDEPTH_MAP]->GetShaderResourceViewRawptr());
	...
	// Deferred 버퍼들을 하나로 뭉쳐서 IBL 연산이 끝난 FrameBuffer로 만든다
	{
		...
		m_Deferred->CombineRenderTargetOpaque(
			...
			m_SSAOManager->AmbientSRV().Get(), m_ViewTM);
	...
	if (_transparentSize > 0)
	{
		...
		m_Deferred->CombineRenderTargetTransparent(
			...
			m_LightManager->GetTextureLightShaderResourceView(0), m_ViewTM);
...
void Renderer::DecalDraw(const unsigned int textureIndex, const DirectX::XMMATRIX& worldTM)
{
	// 디퍼드 컨텍스트로 필요한 그림을 그려서 커맨드라인을 통해 넘겨주자
	auto _texture = m_pResourceManager->GetTexture(textureIndex);
	m_SSDManager->AddSSD(_texture, worldTM);
}
...
void Renderer::SetLoadingSceneImage(
	const std::vector<std::string>& fileName_V, 
	const std::string& vertexShaderPath, 
	const std::string& pixelShaderPath, 
	float width, float height)
{
	m_LoadingScene->SetImageAndFps(m_pDevice, fileName_V, vertexShaderPath, pixelShaderPath, width, height);
}
...
void Renderer::DeferredPickingPass()
{
	...
	ID3D11ShaderResourceView* _srv[6] =
	{
		...
		m_Deferred->GetTransparentDrawLayers()[NORMALDEPTH_MAP]->GetShaderResourceViewRawptr(),
	...
	float aspect = (float)m_ClientWidth / (float)m_ClientHeight;
	float halfHeight = m_CameraFarZ * tanf(0.5f * m_CameraFovY);
	float halfWidth = aspect * halfHeight;

	DirectX::SimpleMath::Matrix _invView = m_ViewTM;
	_invView = _invView.Invert();

	float _pixelPicking[8 + 16] =
	{
	...
	ConstantBufferManager::SetGeneralCBuffer(m_pDeviceContext, CBufferType::CS, CBufferSize::SIZE_24, 0, _pixelPicking);
	m_PixelPicker->PixelPickingFromSRV<DeferredPixel>(
		m_pDeviceContext, m_ClientWidth, m_ClientHeight, _srv, 6, m_DeferredPixelData.get());
	...
	ID3D11DepthStencilState* DepthStencilState)
{
	...
	m_pDeviceContext->PSSetShader(PS_SHADER(eSHADER::PS_2D_NoGammaCor)->GetPixelShader(), NULL, 0);
...
void Renderer::EndDraw()
{
	...
	// UI를 묶는다
	CombineRenderTarget(m_pUIRenderTarget, RasterizerState::GetAlphaBlenderStateUICombine(), nullptr);
