			...
			if (_numCheck_B == true)
			{
				...
				if (_numcheck == L"M")
				{
					...
					for (unsigned int i = 0; i < (unsigned int)eQuestContent::Count; ++i)
					{
						...
						case eQuestContent::TriggerNum:
							_oneQuestBlock.triggerNum = std::stoi(_cell);
							break;
				...
				else if (_numcheck == L"S")
				{
					...
					for (unsigned int i = 0; i < (unsigned int)eQuestContent::Count; ++i)
					{
						...
						{
						case eQuestContent::TriggerNum:
							_oneQuestString.triggerNum = std::stoi(_cell);
							break;
						case eQuestContent::Text:
	...
	if (_file.is_open())
	{
		while (std::getline(_file, _line))
		{
			...
			if (_numCheck_B == true)
			{
				...
				if ((_DialogueBlock.size() > 0 && _DialogueBlock.back().triggerNum != _nowTriggerNum) || _DialogueBlock.size() == 0)
					//if (_DialogueBlock.back().TriggerIdx != _nowTriggerNum || _DialogueBlock.size() == 0)
				{
					DialogueBlock _oneBlock;
					_oneBlock.triggerNum = _nowTriggerNum;
				...
				for (unsigned int i = 0; i < (unsigned int)eDialogueContent::Count; ++i)
				{
					...
					case eDialogueContent::TriggerNum:
						_oneString.triggerNum = std::stoi(_cell);
						break;
