...
void Billboard::DecreaseX(float numerator, float denominator)
{
	DirectX::SimpleMath::Vector3 _Scale = m_originScale;

	_Scale.x = m_originScale.x * (denominator - numerator) / denominator;
	_Scale.y = m_originScale.y * (denominator - numerator) / denominator;
...
void Billboard::Y_FixRender()
{
	...
	XMMATRIX translateMatrix = XMMatrixTranslation(m_pMyObject->m_Transform->m_Position.x + (_ScaleX * m_PivotX), m_pMyObject->m_Transform->m_Position.y + (_ScaleY * m_PivotY), m_pMyObject->m_Transform->m_Position.z);
...
void Billboard::ObserverUpdate(std::shared_ptr<IResourceManager> pResourceManager)
{
	...
	// 이미지의 크기정보
	pResourceManager->GetSpriteScale(m_SpriteIndex_V[0], m_originScale);

	//DirectX::SimpleMath::Vector3 _scale = m_originScale;

	m_originScale.x = m_originScale.x * m_ProportionX;
	m_originScale.y = m_originScale.y * m_ProportionY;

	m_pMyObject->m_Transform->SetScale(m_originScale);
