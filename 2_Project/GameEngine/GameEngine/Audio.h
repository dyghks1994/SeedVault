...
public:
	...
	_DLL SoundEvent* PlayEvent(const char* path, bool duplicatePlay = true);
	_DLL SoundEvent* PlayEvent(const char* path, const char* parameter, float parameterValue, bool duplicatePlay = true);
	_DLL float GetEventDistance(const char* path);
	_DLL void AllEventStop(bool fadeout=true);
