...
{
	//이전 오브젝트 매니저에서 오브젝트가 남아있다면 태그에 추가한다.
	if (!m_pContinualGameObject_V.empty())
	{
		for (int index=0; index< m_pContinualGameObject_V.size(); index++)
		{
			AddToTagManager(m_pContinualGameObject_V[index]->GetTag(), m_pContinualGameObject_V[index]);
		}
	}
}
...
void ObjectManager::ReleaseObject(GameObject* object)
{
	...
	RemoveToTagManager(object->GetTag(), object);
...
GameObject* ObjectManager::FindContinualGameObect(const std::string& objectName)
{
	for (GameObject*& _go : m_pContinualGameObject_V)
	{
...
GameObject* ObjectManager::FindContinualGameObect(const unsigned int& objectId)
{
	for (GameObject*& _go : m_pContinualGameObject_V)
	{
...
void ObjectManager::RemoveToTagManager(const std::string& Tag, GameObject* object)
{
	if (Tag == "Untagged")
	{
		//Untagged는 예외
	...
	{
		for (int index = 0; index < m_TagObject_M[Tag].size(); index++)
		{
			if (m_TagObject_M[Tag][index]==object)
			{
				m_TagObject_M[Tag].erase(m_TagObject_M[Tag].begin()+index);
				return;
			}
		}
		Trace("ObejctManager : Tag Erase Fail");
	}
}
