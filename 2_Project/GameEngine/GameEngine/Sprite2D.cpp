...
void Sprite2D::Start()
{
	...
	DirectX::SimpleMath::Vector3 _scale = DirectX::SimpleMath::Vector3();
	...
	case eUIAxis::BackGround:
		_Xpos = m_width / 2 + m_pMyObject->m_Transform->m_Position.x;
		_Ypos = m_height / 2 + m_pMyObject->m_Transform->m_Position.y;


		_scale.x = m_OriginScale.x * m_width / m_OriginScale.x;
		_scale.y = m_OriginScale.y * m_height / m_OriginScale.y;

		m_pMyObject->m_Transform->SetScale(_scale);
		break;

	case eUIAxis::Credit:
		m_pMyObject->m_Transform->m_Position.x = (m_width - (m_OriginScale.x * m_ProportionX)) / 2;
		m_pMyObject->m_Transform->m_Position.y = 0;
		_Xpos = m_pMyObject->m_Transform->m_Position.x;
		_Ypos = m_pMyObject->m_Transform->m_Position.y;
		break;
	}
...
void Sprite2D::OnUIRender()
{
	...
	case eUIAxis::Credit:
		...
		if (m_pMyObject->m_Transform->m_Position.y < -(m_OriginScale.y * m_ProportionY - m_height))
		{
			m_pMyObject->m_Transform->m_Position.y = -(m_OriginScale.y * m_ProportionY - m_height);
		}
		_Xpos = m_pMyObject->m_Transform->m_Position.x;
		_Ypos = m_pMyObject->m_Transform->m_Position.y;
		break;
...
void Sprite2D::ObserverUpdate(std::shared_ptr<IResourceManager> pResourceManager)
{
	...
	m_OriginScale = DirectX::SimpleMath::Vector3(0, 0, 0);
	DirectX::SimpleMath::Vector3 _scale = DirectX::SimpleMath::Vector3();

	// 이미지의 사이즈를 가져온다
	pResourceManager->GetSpriteScale(m_SpriteIndex_V[m_NowSpriteIndex], m_OriginScale);

	if (m_UIAxis != eUIAxis::BackGround)
	{
		_scale.x = m_OriginScale.x * m_ProportionX;
		_scale.y = m_OriginScale.y * m_ProportionY;
	}
