	...
	// test용은 초기화 안했음
{
	...
	m_MouseTM = DirectX::SimpleMath::Matrix::Identity;
	m_MouseTM._11 = 1.f; m_MouseTM._22 = 1.f;
...
void Engine::Initialize(int hWND, int width, int height)
{
	...
	std::thread _t1(&Engine::ShowLoadingScene, this, std::ref(isDataLoad));
	...
	m_MouseIndex = m_pResourceManager->GetTextureIndex("Game_Logo_line_size.png");
...
{
	std::vector<std::string> _files_V;
	_files_V.push_back("../Data/Image/LoadingScene/Loading_01.png");
	_files_V.push_back("../Data/Image/LoadingScene/Loading_02.png");
	_files_V.push_back("../Data/Image/LoadingScene/Loading_03.png");
	_files_V.push_back("../Data/Image/LoadingScene/Loading_04.png");
	_files_V.push_back("../Data/Image/LoadingScene/Loading_05.png");
	_files_V.push_back("../Data/Image/LoadingScene/Loading_06.png");

	m_pRenderer->SetLoadingSceneImage(_files_V, "../Data/Shader/vs_2d.cso", "../Data/Shader/ps_2d.cso", m_ClientWidth, m_ClientHeight);
	// 로딩씬을 그린다.
...
void Engine::RenderAll()
{
	...
	if (m_bShowMouseCorsor == true)
	{
		m_MouseTM = XMMatrixScaling(40.f, 40.f, 1.0f) *
			XMMatrixRotationRollPitchYaw(0, 0 + XM_PI, 0 + XM_PI) *
			XMMatrixTranslation(m_pInput->GetMouseClientPos().x, m_pInput->GetMouseClientPos().y, 0.5f);

		m_pRenderer->DrawSprite(eResourceType::eSingleImage, m_MouseIndex, m_MouseTM);
	}
...
void Engine::DestroySeedVault()
{
	DestroyWindow(m_hWnd);
}

void Engine::ShowMouseCorsor(bool val)
{
	m_bShowMouseCorsor = val;
}
