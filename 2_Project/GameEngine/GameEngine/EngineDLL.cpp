...
namespace DLLEngine
{
	...
	void DestroySeedVault()
	{
		g_Engine->DestroySeedVault();
	}
	...
	_DLL GameObject* FindContinualGameObjectByName(const char* objectName)
	{
		return Managers::GetInstance()->GetSceneManager()->GetNowScene()->m_pGameObjectManager->FindContinualGameObect(std::string(objectName));
	}

	_DLL GameObject* FindContinualGameObjectById(const unsigned int& objectId)
	{
		return Managers::GetInstance()->GetSceneManager()->GetNowScene()->m_pGameObjectManager->FindContinualGameObect(objectId);
	}

	_DLL void ShowMouseCursor(bool val)
	{
		g_Engine->ShowMouseCorsor(val);
	}
...
namespace DLLGameObject
{
	...
	_DLL const char* GetObjectName(ComponentBase* pComp)
	{
		return pComp->GetMyObject()->GetObjectName_char();
	}
