...
public:
	...
	_DLL void SetClickFunc(std::function<void()> onOver);
	_DLL void SetOverFunc(std::function<void()> onOver);
	...
	_DLL void SetIntersectCheck(const bool& isIntersect);
...
private:
	...
	eButtonState m_PrevState = eButtonState::OFF;
	bool m_IsPushedBefore = false;
	bool m_IsPushed = false;

	bool m_AbleOver = true;
	bool m_isIntersect = true;
	...
	std::function<void()> m_onClickSound;
