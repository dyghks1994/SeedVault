...
void Button::OnUIRender()
{
	...
	case eButtonState::PUT:
		if (m_onClickSound && m_IsPushed == true && m_IsPushedBefore == false)
		{
			m_onClickSound();
		}
	...
	case eButtonState::OVER:
		...
		if (m_onOver && m_PrevState != eButtonState::OVER && m_PrevState != eButtonState::PUT)
		{
			m_onOver();
		}
...
_DLL void Button::SetClickFunc(std::function<void()> onClick)
{
	m_onClickSound = onClick;
}

_DLL void Button::SetOverFunc(std::function<void()> onOver)
{
	m_onOver = onOver;
}
...
void Button::SetIntersectCheck(const bool& isIntersect)
{
...
void Button::ObserverUpdate(std::shared_ptr<IResourceManager> pResourceManager)
{
	...
	m_OverSpriteIndex = pResourceManager->GetSpriteDataIndex(m_OnSprite);
